...
class Macd(StrategyTemplate):
    ...
    def __init__(
        ...
        self.order_info_cols: dict[str, str] = {
            ...
            "gateway_name": 'string',
            # 以下列是策略独有的
            "loop_time": 'datetime64[ns, Asia/Shanghai]',
        }
    ...
    def on_init(self) -> None:
        ...
        # 初始化 self.results
        # TODO 将 CFFEX 也纳入到本策略的负责范围内，参考 class Combined
    ...
    def update_option_data(self, vt_symbol: str, tick: TickData) -> None:
        ...
        self.option_update[vt_symbol] = {
            ...
            'option_openPrice': tick.open_price,
    ...
    def update_future_data(self, vt_symbol: str, tick: TickData) -> None:
        ...
        self.future_update[vt_symbol] = {
            ...
            'future_averagePrice': tick.average_price,
        }
    ...
    def calc_signal(self, group: DataFrame) -> tuple[dict, DataFrame]:
        ...
        try:
            ...
            processed_results['diff'] = processed_results.apply(
    ...
    def exec_signal(self, results_dict: dict[str, dict], target_option: DataFrame) -> None:
        ...
        try:
            self.close_positions_for_principal_protection(results_dict)
            self.close_positions_for_risk_ctrl(results_dict)
            self.close_positions_for_end(results_dict)
            self.close_positions_for_loop_risk_ctrl_and_end(results_dict)
            self.open_positions(target_option)
        except Exception:
                ...
                for _, row in group.iterrows():
                    if self.open_condition(row, str(product_type)):
                        try:
                            ...
                            memo: str = f'Open{self.order_count}'
    ...
    def close_positions_for_principal_protection(self, results_dict: dict[str, dict]) -> None:
        """
        保本平仓(Principal-Protection)
        """
        total_positions = self.main_engine.get_all_positions()
    ...
    def close_positions_for_end(self, results_dict: dict[str, dict]) -> None:
        """14:50平仓"""
        if self.current_time.time() >= datetime.strptime('14:50:00', '%H:%M:%S').time():
            total_positions = self.main_engine.get_all_positions()
        ...
        for close in closable_positions:
            ...
            try:
                ...
                if (self.is_trading_time() 
                    ...
                    and 
                    self.check_option_condition(data, open_price)):
                    if self.close_positon_cooldown.test():
                        # 获取有效买一价，如果不存在或为0则使用最小价格单位，并确保不低于最小价格单位
                        bid_price = max(data.get('option_bidPrice1', data['price_tick']), data['price_tick'])
                        self.avoid_self_dealing(vt_symbol, Direction.LONG, bid_price)
                ...
                #     volume_list = self.split_volume(int(data['max_volume']), close_available_volume)
                #     for sub in volume_list:
                #         price = round(0.5 * open_price / data['price_tick'] + 1) * data['price_tick']
                #         self.request_close_position(vt_symbol, Direction.LONG, price, sub, str(self.order_count))
                #         self.write_log(f"平仓{self.order_count} 合约={vt_symbol} 方向={Direction.LONG} 手数={sub} @{price}")
            except Exception:
    ...
    def close_positions_for_loop_risk_ctrl_and_end(self, results_dict: dict[str, dict]) -> None:
        """循环风控"""
        ...
        try:
            orders_to_process = self.order_info[
                (self.order_info['loop_time'].notna()) &
                (self.order_info['status'].isin([Status.NOTTRADED, Status.PARTTRADED]))
            ]
    ...
    def on_bars(self, bars: dict[str, BarData]) -> None:
        ...
        该回调函数会被两个系统调用：
        - self.pbg 也就是组合K线合成器在合成出一根1分钟K线时会调用
        - self.load_bars 也就是加载历史数据时会调用
        """
        # 类似 self.on_tick()，我们应该只把期货合约喂给 PBG，所以得先把非期货合约过滤掉
        bars = {vt_underlying_symbol: bar for vt_underlying_symbol, bar in bars.items() if vt_underlying_symbol in self.future_vt_symbols}
        # 然后再把K线喂给 PBG
        self.pbg.update_bars(bars)
    
    def on_5minute_bars(self, bars: dict[str, BarData]) -> None:
        """5分钟K线回调"""
        # 更新K线计算MACD数值
        for vt_underlying_symbol, bar in bars.items():
            am: ArrayManager = self.ams[vt_underlying_symbol]
            am.update_bar(bar)

        for vt_underlying_symbol, bar in bars.items():
            am = self.ams[vt_underlying_symbol]
            if not am.inited:
                return
