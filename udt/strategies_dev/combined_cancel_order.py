...
class CombinedCancelOrder(StrategyTemplate):
    """"""
        ...
        for exchange_id in self.results['exchange'].unique():
            if len(previous_dates) == 2:
                ...
                try:
                    ...
                    # 捕获异常并回退到本地数据库
                    self.output(f"获取数据失败，错误信息: {e}. 尝试从本地数据库加载数据...")
                    by_day = pd.to_datetime(by_day, format='%Y%m%d')
                    y_day = pd.to_datetime(y_day, format='%Y%m%d')

                    df = pd.read_csv(f'C:/python/exchange_data/{exchange_id.lower()}.csv')
                ...
                self.output("无法获取前两个交易日的历史最高最低价数据")
