...
from vnpy.trader.utility import get_file_path, ArrayManager
...
class MacdStrategy(StrategyTemplate):
    ...
    def __init__(
        ...
        self.op1: LoopRiskCtrlOperation = LoopRiskCtrlOperation(self)

    def on_init(self) -> None:
        """策略初始化"""

        self.pbg = PortfolioBarGenerator(self.on_bars, 5, self.on_5minute_bars, Interval.HOUR)
    ...
    def calc_signal(self, group: DataFrame) -> tuple[dict, DataFrame]:
        ...
        try:
            ...
            processed_results['target_option_rank'] = processed_results.groupby(['option_type', 'underlying_vt_symbol'], sort=False)['diff'].rank()
